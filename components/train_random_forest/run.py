...
def delta_date_feature(dates):
    """
    Given a 2d array containing dates (in any format recognized by pd.to_datetime), retrn the delta in days
...
def go(args):
    ...
    # Fix random seed for Random Forest, so reproducible results are obtained
    rf_config['random_state'] = args.random_seed

    # Use run.use_artifact(...).file() to get the train and validation artifact
    # and save returned path in train_local_path
    trainval_local_path = run.use_artifact(args.trainval_artifact).file()
   
    X = pd.read_csv(trainval_local_path)
    y = X.pop("price")  # removes column "price" from X and puts it into y
    ...
    # Then fit to X_train, y_train data
    logger.info("Fitting")

    ######################################
    # Fit pipeline sk_pipe by calling .fit method for X_train and y_train
    ...
    ######################################
    # Save sk_pipe pipeline as a mlflow.sklearn model in "random_forest_dir" 
    ...
    # Upload model just exported to W&B
